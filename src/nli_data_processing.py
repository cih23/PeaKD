...
from src.modeling import BertForSequenceClassificationEncoder, BertForSequenceClassificationEncoder_SPS, BertForSequenceClassificationEncoder_SPS_6layer_student, FCClassifierForSequenceClassification
...
#         teacher_summary = f'~/KDAP/data/outputs/KD/QNLI/QNLI_patient_kd_teacher_12layer_result_summary_2.pkl'
    ...
    if types.lower() == 'train':
        ...
        dataloader = DataLoader(dataset, sampler = RandomSampler(dataset), batch_size = 192)
...
def init_glue_model_SPS(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
...
def init_glue_model_SPS_6layer_student(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_SPS_6layer_student(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier

# def init_glue_model_U(task_name, output_all_layers, num_hidden_layers, config):
#     logger.info('predicting for %s' % task_name.upper())
#     processor = processors[task_name.lower()]()
#     output_mode = output_modes[task_name.lower()]
...
#     encoder_bert = BertForSequenceClassificationEncoder_U(config, output_all_encoded_layers=output_all_layers,
#                                                         num_hidden_layers=num_hidden_layers)
#     classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
#     return encoder_bert, classifier
...
def init_pretrain_model_PTP_SPS(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('Initializing model for pretraining')
    mode = None
    num_labels_1 = 4
    encoder_bert = BertForSequenceClassificationEncoder_SPS(config, output_all_encoded_layers=output_all_layers,
...
def init_pretrain_model_PTP_SPS_6layer_student(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('Initializing model for pretraining')
    mode = None
    num_labels_1 = 4
    encoder_bert = BertForSequenceClassificationEncoder_SPS_6layer_student(config, output_all_encoded_layers=output_all_layers,
