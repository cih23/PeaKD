...
from src.nli_data_processing import processors, output_modes, init_glue_model_SPS_6layer_student, init_glue_model_SPS
...
from src.utils import load_model, count_parameters, eval_model_dataloader_nli, eval_model_dataloader, load_model_wonbon, compute_metrics
...
if args.kd_model.lower() in ['kd', 'kd.cls', 'kd.u', 'kd.i']:
    ...
    # if SPS model for 1,2,3-layer student.
        ...
        student_encoder, student_classifier = init_glue_model_SPS_6layer_student(task_name, output_all_layers, args.student_hidden_layers, student_config)
    else:
        student_encoder, student_classifier = init_glue_model_SPS(task_name, output_all_layers, args.student_hidden_layers, student_config)
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.kd_model.lower() in['kd', 'kd.cls', 'kd.u', 'kd.i']:
                ...
                elif args.kd_model.lower() in ['kd.u', 'kd.i']:
                    student_patience = torch.stack(full_output[1:10:2]).transpose(0, 1)
                else:
            ...
            if (epoch == 5):
                logger.info('='*77)
                logger.info("Best f1 : "+ str(eval_best_f1)+ "Best acc : "+str(eval_best_acc))
                raise ValueError('Skip epoch 6')
        #Save a trained model and the associated configuration
            if (global_step % 1 == 0) & (epoch >-1): 
                if 'race' in task_name:
                    result = eval_model_dataloader(student_encoder, student_classifier, eval_dataloader, device, False)
                else:
                    test_res = eval_model_dataloader(student_encoder, student_classifier, eval_dataloader, args.device, detailed=True, verbose=False)
                ...
                if result_['f1'] > eval_best_f1:
                    logger.info("")
                    logger.info('='*77)
                    logger.info("Validation f1 improved! "+str(eval_best_f1)+" -> "+str(result_['f1']))
                    logger.info("Validation Acc : "+ str(result_['acc']))
                    logger.info('='*77)
                ...
                if result_['acc'] > eval_best_acc:
                    logger.info("")
                    logger.info('='*77)
                    logger.info("Validation Acc improved! "+str(eval_best_acc)+" -> "+str(result_['acc']))
                    logger.info('='*77)
                    eval_best_acc = result_['acc']
                if test_res['loss']< eval_loss_min:
                    logger.info("")
                    logger.info('='*77)
                    logger.info("Validation Loss improved! "+str(eval_loss_min)+" -> "+str(test_res['loss']))
                    logger.info('='*77)
...
logger.info("")
logger.info('='*77)
logger.info("Validation Accuracy : "+str(eval_best_acc)+" Validation f1 : "+str(eval_best_f1))
